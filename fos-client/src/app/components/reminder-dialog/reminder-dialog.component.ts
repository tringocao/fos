...
export class ReminderDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.orderService.GetUserNotOrdered(this.data.event.EventId).then(result => {
      ...
      this.data.eventSource.paginator = this.paginator;
    });
  }
  ...
  closeEvent($event) {
    this.summaryService.updateEventStatus(this.data.event.EventId, "Closed").then(response => {
      if (response === null) {
