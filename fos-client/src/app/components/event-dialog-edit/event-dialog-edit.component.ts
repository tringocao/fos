...
export class EventDialogEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.data.RemindTime) {
      ...
      this.dateToReminder = remindTime;
    }
  ...
  ValidateEventRemindTime(
    ...
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      console.log(
        remindDate + ' ' + moment(remindDate).isSameOrAfter(eventDate)
      );
  ...
  ValidateEventCloseTime(
    ...
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      ...
      if (eventDate && closeDate) {
        if (moment(closeDate).isSameOrAfter(eventDate)) {
          console.log('eeee');
          // this.isInvalidCloseTime = true;
          this.ownerForm.controls['dateTimeToClose'].setErrors({
            invalidCloseTime: true
          });
          this.ownerForm.controls['dateTimeToClose'].hasError(
            'invalidCloseTime'
          );
  ...
  onDateTimeChange(value: string): void {
    if (
      this.dateToReminder &&
      this.dateEventTime &&
      moment(this.dateEventTime).isSameOrAfter(this.dateToReminder)
    ) {
    ...
    if (
      ...
      moment(this.dateToReminder).isSameOrAfter(this.dateTimeToClose)
    ) {
      // this.isInvalidRemindTime = true
      // this.ownerForm.controls["dateTimeToClose"].setErrors({invalidCloseTime: true})
    ...
    if (
      this.dateTimeToClose &&
      this.dateEventTime &&
      moment(this.dateTimeToClose).isSameOrAfter(this.dateEventTime)
    ) {
      // this.isInvalidCloseTime = true
      this.ownerForm.controls['dateTimeToClose'].setErrors({
        invalidCloseTime: true
